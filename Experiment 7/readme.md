...
Why storing plain passwords in a database is unsafe.
What the limitation of simple hashing is.
How salting improves password storage security.
How a Python web application can interact with a MySQL database using XAMPP.
The application uses Flask for the web framework and MySQL for the database. MySQL is expected to run through XAMPP.
The app includes:
A registration page for hash-only registration.
A registration page for hashing with salt.
A login page that verifies stored credentials.
A dashboard page to inspect stored data in the database.
A delete action to remove users from the database.
Automatic database and table creation if they do not already exist.
The central educational goal of the app is to show that:
Passwords should not be stored directly.
Hashing transforms a password into a fixed-length digest.
Hashing alone still has weaknesses.
Adding a random salt makes stored password hashes stronger and more unique.
...
Hashing alone still has weaknesses.
For example:
If two users choose the same password, they get the same hash.
Weak passwords can still be guessed with dictionary attacks.
Attackers can use precomputed hash lists or rainbow tables.
...
Even if two users choose the same password, their stored hash can be different.
It makes precomputed attacks much less useful.
It adds uniqueness per user.
...
Flask
MySQL
XAMPP for MySQL service access
HTML templates through Jinja2
CSS for styling
...
Navigation links.
Flash message area.
Main content block used by all child templates.
...
The table of stored users from MySQL.
...
`secrets` to generate secure random salts.
`Flask` and helper functions from `flask`.
`mysql.connector` to connect Python to MySQL.
`Error` to catch MySQL exceptions.
6.2 Flask App Setup
...
Connection without selecting a database, used when the database may not exist yet.
Connection with the database selected, used for normal app operations.
...
Important detail:
The password is encoded as UTF-8 bytes.
`hashlib.sha256(...)` processes the bytes.
`.hexdigest()` returns a 64-character hexadecimal string.
...
Method: `GET`
Purpose:
Redirect user to the first registration page.
Start the demo from hash-only registration.
`/dashboard`
Method: `GET`
Purpose:
...
Methods: `GET`, `POST`
Purpose:
Show the hash-only registration form.
On submit, validate inputs.
Hash the password with SHA-256.
Store username, hash, `NULL` salt, and method `hash_only`.
`/register/salted`
Methods: `GET`, `POST`
Purpose:
Show the salted registration form.
On submit, validate inputs.
Generate a random salt.
Hash the salt + password.
Store username, salted hash, salt, and method `hash_with_salt`.
`/login`
Methods: `GET`, `POST`
Purpose:
Show login form.
...
Check which method was used for storage.
Recreate the correct hash from the submitted password.
Compare recreated hash with stored hash.
Show success or failure message.
`/delete/<int:user_id>`
Method: `POST`
Purpose:
Delete a selected user from the database.
Show feedback with flash messages.
6.11 App Startup
...
```
This means:
When `app.py` is run directly, the database is initialized first.
Then Flask starts in debug mode.
...
A flash-message display section.
A Jinja block called `content`.
7.2 Dashboard Template
`index.html` is the main explanation and inspection page.
It includes:
A project summary card.
A database status card.
A section about hashing limitations.
A section about salting improvements.
A database table of current users.
...
Password is hashed directly.
Same password can produce same hash.
If the mode is `hash_with_salt`, the page explains:
A salt is created for the user.
Password is combined with salt before hashing.
Same password can still produce different stored hashes.
7.4 Login Template
...
Monospace formatting for hash and salt columns.
...
Purpose:
Stores the SHA-256 hexadecimal hash.
SHA-256 hex output length is 64 characters.
`salt`
Type: `CHAR(32) NULL`
Purpose:
Stores the generated salt for salted users.
Remains `NULL` for hash-only users.
`method`
Type: `ENUM('hash_only', 'hash_with_salt') NOT NULL`
Purpose:
Records how the password was stored.
Allows login logic to know which verification method to use.
`created_at`
Type: `TIMESTAMP DEFAULT CURRENT_TIMESTAMP`
Purpose:
Saves when the record was created.
9. Request Flow in Practice
9.1 Hash-Only Registration Flow
User opens `/register/hash`.
User enters username and password.
Flask receives the form submission.
The app checks that both fields are non-empty.
`sha256_hash(password)` is called.
The generated hash is inserted into MySQL.
The `salt` column is saved as `NULL`.
The `method` column is saved as `hash_only`.
User is redirected to the dashboard.
9.2 Salted Registration Flow
User opens `/register/salted`.
User enters username and password.
Flask validates inputs.
The app generates a random salt using `secrets.token_hex(16)`.
The app calls `hash_with_salt(password, salt)`.
The salted hash and salt are stored in MySQL.
The `method` column is saved as `hash_with_salt`.
User is redirected to the dashboard.
9.3 Login Flow
User opens `/login`.
User submits username and password.
Flask loads the matching user row from MySQL.
The app reads the `method` field.
If method is `hash_only`, it hashes the submitted password directly.
If method is `hash_with_salt`, it hashes `salt + password`.
It compares the generated hash with the stored hash.
It displays success or failure.
9.4 Dashboard Flow
User opens `/dashboard`.
The app ensures the database exists.
It fetches all user rows.
It renders them in an HTML table.
The user can inspect the stored hash and salt values.
...
Hash values are visible in the database.
Salts can also be stored safely in the database.
Same-password reuse is easier to observe with hash-only mode.
Salting improves resistance to certain attack patterns.
It also connects theory to implementation because the user can:
Register through the web app.
Open phpMyAdmin.
Inspect the exact database table.
Compare stored records.
...
It uses plain SHA-256 for demonstration, not a password hashing algorithm like `bcrypt`, `scrypt`, or `Argon2`.
There is no session-based logged-in user area.
There is no CSRF protection layer added manually.
Input validation is minimal.
Flash messages may expose raw database errors in development.
...
A Flask web app for a software security lab.
MySQL integration through XAMPP.
Automatic database and table creation.
Registration with simple hashing.
Registration with hashing plus salt.
Login verification based on stored method.
A dashboard to inspect hashes and salts in MySQL.
A delete function to remove rows.
A short README and a full detailed README.
16. Suggested Future Enhancements
If this project is extended later, useful next steps could be:
Add a side-by-side comparison page for same-password examples.
Add screenshots to documentation.
Add a route showing intermediate demo values safely for teaching.
Replace SHA-256 with `bcrypt` or `argon2` in an advanced version.
Add test files.
Add better form validation.
Add a real login session after verification.
17. Final Note
...
What exact code is present.
How to run and explain the project.
This makes it suitable as:
A handoff document.
A lab report support file.
A revision document.
A base for future extension.
